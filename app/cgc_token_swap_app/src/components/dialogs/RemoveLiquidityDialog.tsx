...
export function RemoveLiquidityDialog({
  ...
  const removeAmount = useLiquidity((s) => s.removeAmount)
        ...
        >
          ...
          <Row className="flex-col gap-1">
            <Button
              ...
              onClick={() => {
                txRemoveLiquidity().then(({ allSuccess }) => {
                  if (allSuccess) {
                    const { logSuccess } = useNotification.getState()
                    logSuccess('Success', `Remove ${toString(removeAmount)} Lp Tokens`)
                    useLiquidity.setState({ removeAmount: '' })
                    setAmountIsNegative(false)
                    setAmountIsOutOfMax(false)
                    onClose()
