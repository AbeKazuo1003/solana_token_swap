...
export default function useLiquidityUrlParser() {
  ...
  const liquidityPoolInfos = useLiquidity((s) => s.poolInfos)
  ...
  const findLiquidityInfoByAmmId = useCallback(
    (ammid: string) => liquidityPoolInfos.find((jsonInfo) => jsonInfo.id === ammid),
    [liquidityPoolInfos]
  )
  ...
  useAsyncEffect(async () => {
    ...
    if (!pathname.includes('/liquidity')) return
    ...
    if (urlAmmId || (urlCoin1Mint && urlCoin2Mint)) {
      ...
      if (matchedMarketJson) {
        useLiquidity.setState({
          currentPoolInfo: matchedMarketJson,
          ammId: matchedMarketJson!.id
        })
      } else if (urlAmmId) {
        // may be just haven't load liquidityPoolJsonInfos yet
        if (liquidityPoolInfos.length > 0) logWarning(`can't find Liquidity pool with url ammId`)
      }
    }

    // if not load enough data(no liquidity pools or no tokens), do not change flag
    if (liquidityPoolInfos.length > 0 && Object.keys(tokens).length > 0) {
